...
const dummyNonAcademicAdmin = {
  name: "Pushpa Rani",
  adminId: "NONACAD2025",
  email: "pushpa.rani@dseu.ac.in",
  phone: "+91 9876545678",
...
export default function NonAcademicAdminProfilePage() {
  return (
    <div className="min-h-screen w-full p-4 sm:p-6 bg-gradient-to-br from-blue-50/50 via-white to-purple-50/30">
      <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="flex flex-col md:flex-row md:items-center md:gap-6 gap-2 text-blue-100 mb-4">
                <span className="flex items-center justify-center md:justify-start gap-2">
                  ...
                  <span className="text-lg font-medium">{dummyNonAcademicAdmin.adminId}</span>
                </span>
                <span className="flex items-center justify-center md:justify-start gap-2">
                  <ShieldCheck className="w-5 h-5" />
                  <span className="text-lg font-medium">{dummyNonAcademicAdmin.role}</span>
                </span>
              </div>
              {/* Quick Stats */}
              <div className="grid grid-cols-2 md:grid-cols-2 gap-4 mt-6">
                <div className="text-center p-3 bg-white/10 rounded-lg backdrop-blur-sm">
                  <div className="text-2xl font-bold text-white">{dummyNonAcademicAdmin.permissions}</div>
                  <div className="text-sm text-blue-200">Permissions</div>
                </div>
                <div className="text-center p-3 bg-white/10 rounded-lg backdrop-blur-sm">
                  <div className="text-2xl font-bold text-white">{dummyNonAcademicAdmin.managedUsers}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <ProfileField icon={<Mail />} label="Email Address" value={dummyNonAcademicAdmin.email} />
            <ProfileField icon={<Phone />} label="Phone Number" value={dummyNonAcademicAdmin.phone} />
          </InfoCard>
          {/* Work Information */}
          <InfoCard 
          ...
          >
            <ProfileField icon={<Building2 />} label="Campus" value={dummyNonAcademicAdmin.campus} />
            <ProfileField icon={<User />} label="Department" value={dummyNonAcademicAdmin.department} />
          </InfoCard>
          ...
          >
            <ProfileField icon={<Calendar />} label="Join Date" value={dummyNonAcademicAdmin.joinDate} />
            <ProfileField icon={<MapPin />} label="Last Login" value={dummyNonAcademicAdmin.lastLogin} />
          </InfoCard>
        </div>
        {/* Privileges Card */}
        <InfoCard 
          title="Non-Academic Privileges" 
