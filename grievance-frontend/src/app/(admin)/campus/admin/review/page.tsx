...
const CampusAdminReviewGrievancesPage = () => {
  useAuthRedirect();
  ...
  useEffect(() => {
    if (!isCampusAdmin) {
      setError("Access denied. Only campus admins can review grievances.");
      setLoading(false);
      return;
    }
  ...
  useEffect(() => {
    ...
    // Filter by status for campus admin review (typically pending or redirected grievances)
    filtered = filtered.filter(
      (g) => g.status === "PENDING" || g.status === "REVIEW"
    );
    ...
    async (isRetry = false) => {
      try {
        ...
        if (response.success) {
          ...
          throw new Error(
            response.message || "Failed to fetch review grievances"
          );
        }
      } catch (err: any) {
        console.error("Error fetching review grievances:", err);
  ...
  const handleReviewGrievance = (grievanceId: string) => {
    router.push(`/campus/admin/issueReview?id=${grievanceId}`);
  };
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="max-w-md mx-auto bg-white rounded-lg shadow-md p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">
            You need campus admin privileges to review grievances.
          </p>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                Review Grievances - Campus Admin
              </h1>
              <p className="text-gray-600 mt-1">
                Review and manage grievances assigned to your campus (
          ...
          {currentGrievances.length === 0 ? (
            <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                No Grievances for Review Found
              </h3>
              <p className="text-gray-600">
                {searchTerm || typeFilter !== "all" || campusFilter !== "all"
                  ? "Try adjusting your filters to see more results."
                  : "No grievances requiring review at the moment."}
              </p>
            </div>
          ) : (
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <button
                      ...
                      className="px-3 py-1 text-purple-600 hover:bg-purple-50 rounded-lg transition-colors flex items-center gap-1"
                    >
...
export default CampusAdminReviewGrievancesPage;
